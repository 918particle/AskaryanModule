...
std::vector<float>* Askaryan_RB::k(){
...
std::vector<float>* Askaryan_RB::eta(){
	std::vector<float> *result = new std::vector<float>;
...
void Askaryan_RB::setAskTheta(float x){
	_askaryanTheta = x;
}

void Askaryan_RB::setAskFreq(std::vector<float> *x){
	_askaryanFreq = x;
}

void Askaryan_RB::setAskR(float x){
	_askaryanR = x;
}

void Askaryan_RB::setAskDepthA(float x){
	_askaryanDepthA = x;
}

void Askaryan_RB::setNmax(float x){
	_Nmax = x;
}

void Askaryan_RB::setAskE(float x){
	_E = x;
}

float Askaryan_RB::getAskE(){
    return _E;
}

float Askaryan_RB::getAskDepthA(){
    return _askaryanDepthA;
}

std::vector<cf>* Askaryan_RB::I_ff(){
...
std::vector<std::vector<cf> >* Askaryan_RB::E_omega(){
...
float Askaryan_RB::criticalF(){
		return *max_element(_askaryanFreq->begin(),_askaryanFreq->end());
}

std::vector<std::vector<float> >* Askaryan_RB::E_t(){
...
std::vector<float>* Askaryan_RB::time(){
	float fc = criticalF();
	float dt = 1.0/(2.0*fc);
...
void Askaryan_RB::emShower(float E){
    this->setAskE(E);
    this->_isEM = 1;
...
void Askaryan_RB::hadShower(float E){
	this->setAskE(E);
    this->_isHAD = 1;
...
void Askaryan_RB::setFormScale(float d){
    _rho0 = d; //rho0 means rho0, not sqrt(2pi)rho0
}

void Askaryan_RB::toggleFormFactor(){
    _useFormFactor = !_useFormFactor;
}

void Askaryan_RB::toggleLowFreqLimit(){
	_strictLowFreqLimit = !_strictLowFreqLimit;
}

float Askaryan_RB::getAskR(){
    return _askaryanR;
}

float Askaryan_RB::getAskEta(float nu){
    return 2.0*3.14159*nu/0.3/_askaryanR*_askaryanDepthA*_askaryanDepthA*sin(_askaryanTheta)*sin(_askaryanTheta);
}

float Askaryan_RB::getAskNmax(){
    return _Nmax;
}

void Askaryan_RB::setIndex(float n){
	INDEX = n;
	COS_THETA_C = 1.0/n;
}
