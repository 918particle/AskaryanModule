...
void Askaryan_ARVZ::setAskTheta(float x)
{
	_askaryanTheta = x;
}

void Askaryan_ARVZ::setAskFreq(std::vector<float>* x)
{
	_askaryanFreq = x;
}

void Askaryan_ARVZ::setAskR(float x)
{
	_askaryanR = x;
}

void Askaryan_ARVZ::setAskE(float x)
{
	_E = x;
	if(_EM)
	{
...
float Askaryan_ARVZ::getAskE(){
    return _E;
}

std::vector<std::vector<cf> >* Askaryan_ARVZ::E_omega()
{
...
float Askaryan_ARVZ::criticalF()
{
		return *max_element(_askaryanFreq->begin(),_askaryanFreq->end());
}

float Askaryan_ARVZ::FormFactor(float t)
{	
...
float Askaryan_ARVZ::A_t(float t)
{	
	float result = 0.0;
...
std::vector<std::vector<float> >* Askaryan_ARVZ::E_t()
{
...
std::vector<float>* Askaryan_ARVZ::time()
{
	return _times;
}

float Askaryan_ARVZ::getAskR()
{
    return _askaryanR;
}

void Askaryan_ARVZ::setIndex(float n){
	INDEX = n;
	COS_THETA_C = 1.0/n;
}

void Askaryan_ARVZ::SetEmHad(bool em,bool had)
{
	_EM = em;
	_HAD = had;
}

void Askaryan_ARVZ::setAskTimes(std::vector<float>* t)
{
	_times = t;
}

std::vector<float>* Askaryan_ARVZ::getAskFreq()
{
	return _askaryanFreq;
}

void Askaryan_ARVZ::emShower()
{
	float E_CRIT = 0.073; //GeV
	//Greisen EM shower profile from Energy E in GeV.
	std::vector<float> *nx = new std::vector<float>;
