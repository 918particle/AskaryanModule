...
std::vector<float>* HansonConnolly::k(){
...
std::vector<float>* HansonConnolly::eta(){
	std::vector<float> *result = new std::vector<float>;
...
std::vector<cf>* HansonConnolly::I_ff(){
...
std::vector<std::vector<cf> >* HansonConnolly::E_omega(){
...
std::vector<std::vector<float> >* HansonConnolly::E_t(){
...
void HansonConnolly::setFormScale(float d){
    _rho0 = d; //rho0 means rho0, not sqrt(2pi)rho0
}

void HansonConnolly::toggleFormFactor(){
...
float HansonConnolly::getAskNmax(){
    return _Nmax;
}
